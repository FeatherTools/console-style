...
let showConsoleExample (console: ConsoleStyle) =
    ...
    console.Message "- service 1 - start"
    let loadingChars = [| '⠋'; '⠙'; '⠹'; '⠸'; '⠼'; '⠴' |]

    for i in 1 .. 10 do
        //output.Message "<c:green>Service 1</c> - <c:blue>%02i</c> - <c:yellow>processing</c> ..." i
        let loadingChar = loadingChars.[i % loadingChars.Length]

        //printf "\r- service 1 - %c" loadingChar
        //Console.Write $"\r- service 1 - {loadingChar}"
        console.Write $"\r- service 1 - {loadingChar}"
        System.Threading.Thread.Sleep(200)

    //Console.WriteLine "\r- service 1 - done"
    //printfn "\r- service 1 - done"
    console.WriteLine "\r- service 1 - done"
